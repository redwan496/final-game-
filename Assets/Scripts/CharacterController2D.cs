...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		...
		{
			DoJump ();
			// disable double jump after double jumping since you can only double jump once
	...
	// do what needs to be done to freeze the player
 	void FreezeMotion() {
		...
		// disable colliders    
		CircleCollider2D circle = _transform.gameObject.GetComponent<CircleCollider2D>();        
		if (circle)            
			circle.enabled = false;
		BoxCollider2D box = _transform.gameObject.GetComponent<BoxCollider2D>();        
		if (box)            
			box.enabled = false;
